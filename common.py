...
class PreDiffractionDataset(Dataset):
    def __init__(self, root_dir, normalize=False):
        """
        in normalize, converts to dimensionless amplitude.
        """
        ...
        print(f"Found {len(self.pre_files)} pre-diffraction and {len(self.diff_files)} diffracted .npz files in {root_dir}")
...
def load_datasets_and_dataloaders(train_dir, in_dist_val_dir, out_dist_val_dir, batch_size=128, num_workers=4, normalize=False):
    train_dataset = PreDiffractionDataset(root_dir=train_dir, normalize=normalize)
    in_dist_val_dataset = PreDiffractionDataset(root_dir=in_dist_val_dir, normalize=normalize)
    out_dist_val_dataset = PreDiffractionDataset(root_dir=out_dist_val_dir, normalize=normalize)
